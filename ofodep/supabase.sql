...
---------------------------------------------------------------------
-- FASE 2: ComercioS (COMERCIOS) Y GESTIÓN DE HORARIOS
---------------------------------------------------------------------
...
-- Define las suscripciones de las comercios, que permiten definir si la 
-- comercio está activa para las búsquedas y para recibir pedidos.
...
-- Se asume que la validación de que el horario de el comercio esté activo, y que totales y
...
2. LECTURA PÚBLICA:
   ...
   • Siempre que la suscripción de el comercio esté activa.

3. ADMINISTRADORES DE ComercioS (STORE ADMINS):
...
5. TABLA "ORDERS":
   ...
   • Un usuario puede insertar un order siempre que:
       ...
       - La suscripción de el comercio esté activa (expiration_date > now()).
       - El horario de el comercio esté activo.
...
BEGIN
    ...
    IF NOT is_admin THEN
        ...
        -- Verifica si el usuario es administrador de el comercio del pedido
...
---------------------------------------------------------------
/*
4. Función auxiliar para verificar si una comercio está abierta para pedidos (antes "store_horario_activo").
---------------------------------------------------------------
*/
...
BEGIN
  ...
  IF exception_record IS NOT NULL THEN
    -- Si se marca como cerrado en la excepción, el comercio está cerrada
...
BEGIN
  ...
  -- Si hay una excepción que indique que el comercio está cerrada hoy, retorna false.
  IF EXISTS (
    SELECT 1
    FROM store_schedule_exceptions
  ...
  -- Si hay una excepción que indique que el comercio está abierta hoy en el horario actual, retorna true.
  IF EXISTS (
    SELECT 1
    FROM store_schedule_exceptions
    WHERE store_id = p.store_id
  ...
  -- Verificar en el horario regular si el comercio debería estar abierta.
  IF EXISTS (
    SELECT 1
...
BEGIN
    ...
    LOOP
        -- Verificar que el producto pertenece a el comercio del pedido
        SELECT store_id INTO prod_store
        FROM products
        WHERE id = prod.product_id;
        
        IF prod_store IS NULL OR prod_store <> NEW.store_id THEN
            RAISE EXCEPTION 'El producto % no pertenece a el comercio del pedido', prod.product_id;
        END IF;
...
-- Permite la lectura si la suscripción de el comercio está activa, o el usuario es administrador
-- global o administrador de el comercio.
...
-- Permite UPDATE solo a administradores de el comercio y únicamente cuando la suscripción esté activa.
...
-- Lectura pública y modificación solo para administradores de el comercio.
...
-- Solo administradores de el comercio pueden insertar, actualizar o eliminar productos.
...
--   - O el usuario es administrador de el comercio.
...
--   - La suscripción de el comercio está activa.
--   - el comercio se encuentra abierta (verificado mediante la función auxiliar store_schedule_active).
...
-- Permite que el usuario propietario del pedido (o un administrador global o administrador de el comercio) pueda actualizar.
...
--   - El usuario es administrador de el comercio.
--   - O es administrador global.
...
DECLARE
  ...
  day_of_week integer;
BEGIN
  -- Obtener la zona horaria de el comercio
  SELECT timezone INTO tz FROM stores WHERE id = p.store_id;

  -- Convertir la hora actual del sistema a la hora local de el comercio
  ...
  -- Excepción: comercio cerrada hoy
  IF EXISTS (
    SELECT 1
    FROM store_schedule_exceptions
    WHERE store_id = p.store_id
  ...
  -- Excepción: comercio abierta hoy en horario especial
  IF EXISTS (
    SELECT 1
    FROM store_schedule_exceptions
    WHERE store_id = p.store_id
